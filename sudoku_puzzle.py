...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for i in range(self._n):
            for j in range(self._n):
                if self._grid[i][j] == EMPTY_CELL and self._helper_fail(i, j):
                    return True
        return False

    def _helper_fail(self, i: int, j: int) -> bool:
        """
        return True if Sudoku, <self>,  has no allowable symbols
        remaining to choose from at the given position, self._grid[i][j].

        pre-condition: 0 <= i < self._n, 0 <= j < self._n
        >>> s = SudokuPuzzle(4, \
        [["B", "D", "A", "C"], \
        ["C", "A", "B", "D"], \
        ["A", "B", " ", " "], \
        ...
        True
        """
        row_set = self._symbol_set.difference(self._row_set(i))
        column_set = self._symbol_set.difference(self._column_set(j))
        subsquare_set = self._symbol_set.difference(self._subsquare_set(i, j))
        intersection = row_set.intersection(column_set, subsquare_set)
    ...
    def has_unique_solution(self) -> bool:
        ...
        """
        solver = BfsSolver()
        first_answer = solver.solve(self)
        if not first_answer:
            return False
        this = {str(first_answer[-1])}
        second_answer = solver.solve(self, this)
