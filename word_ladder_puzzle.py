...
class WordLadderPuzzle(Puzzle):
    ...
    def __eq__(self, other: WordLadderPuzzle) -> bool:
        ...
        True
        """
        return self.to_word == other.to_word and \
            self.from_word == other.from_word and \
            self.word_set == other.word_set

    def __str__(self) -> str:
        """
        Return a human-friendly string representing of this WordLadderPuzzle's
    ...
    def is_solved(self) -> bool:
        ...
        False
        """
        return self.to_word == self.from_word
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        for i in range(len(self.from_word)):
            for j in range(len(self._chars)):
                new_word = self.from_word[:i] + self._chars[j] \
                    + self.from_word[i + 1:]
                if new_word != self.from_word and new_word in self.word_set:
                    extension = WordLadderPuzzle(new_word, self.to_word,
                                                 self.word_set)
