...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        result_add = 0
        result_time = 1
        for each in self._subtrees:
            if self._root == '+':
                result_add += each.eval(lookup)
            else:
                result_time *= each.eval(lookup)
        return result_add if self._root == '+' else result_time
    ...
    def __str__(self) -> str:
        ...
        if self.is_empty():
            ...
            result = ''
            for each in self._subtrees:
                result += each.__str__() + ' ' + self._root + ' '
            result.strip()
            return '(' + result[:-3] + ')'
    ...
    def __eq__(self, other: ExprTree) -> bool:
        ...
        if self.is_empty():
            ...
            return other._subtrees == [] and other._root == self._root
        else:
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    for lst in values[1:]:
        parent = q.dequeue()
        for each in lst:
            if isinstance(each, str) and not each.isalpha():
                child = ExprTree(each, [])
                parent.append(child)
                q.enqueue(child)
            else:
