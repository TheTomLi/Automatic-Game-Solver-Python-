...
class DfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        for item in puzzle.extensions():
            # check whether it has been seen before or solvable
            if str(item) in seen:
                pass
            elif item.fail_fast():
                seen.add(str(item))
            else:
                solution = self.solve(item, seen)
                if solution:
...
class BfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        if seen is None:
            seen = set(str(puzzle))
        elif str(puzzle) in seen or puzzle.fail_fast():
            return []
        q.enqueue([puzzle])
        while not q.is_empty():
            ...
            if curr[-1].is_solved():
                return curr
            extension_lst = curr[-1].extensions()
            for each in extension_lst:
                if str(each) in seen:
                    pass
                elif each.fail_fast():
                    seen.add(str(each))
